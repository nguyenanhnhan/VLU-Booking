...
class LecturerAutoCompletePage extends Page
{
    private $listMethods = [];
    ...
    public function __construct()
    {
        ...
        $this->listMethods[LecturerAutoCompleteType::Lecturer] = 'GetLecturers';
        $this->listMethods[LecturerAutoCompleteType::MyLecturers] = 'GetMyLecturers';
        $this->listMethods[LecturerAutoCompleteType::Group] = 'GetGroups';
        $this->listMethods[LecturerAutoCompleteType::Organization] = 'GetOrganizations';
    }
    ...
    /**
     * @param $term string
     * @return array|AutocompleteLecturer[]
     */
    private function GetLecturers($term)
    {
        if ($term == 'group') {
        ...
        $filter = new SqlFilterLike(ColumnNames::LECTURER_ID, $term);
        ...
        $results = $r->GetLecturerList(1, PageInfo::All, null, null, $filter)->Results();
        ...
        foreach ($results as $result) {
            if (!$hideUserDetails || $result->Id == $currentUser->UserId || $user->IsGroupAdminFor($result->GroupIds) || $currentUser->IsAdmin) {
                $users[] = new AutocompleteLecturer($result->Id, $result->Email, $result->FullName);
            }
        }
    ...
    private function GetMyLecturers($term)
    {
        ...
        if (!empty($groupIds)) {
            ...
            foreach ($results as $result) {
                ...
                $users[$result->Id] = new AutocompleteLecturer($result->Id, $result->First, $result->Last, $result->Email, $result->FullName);
            }
        }
...
class LecturerAutoCompleteType
{
    public const Lecturer = 'lecturer';
    public const Group = 'group';
    public const MyLecturers = 'myLecturers';
